...
void PassiveMouseMotion(int x, int y) {
   ...
   /* invert y axis */
   iY = ImageHeight - y + (glutGet(GLUT_WINDOW_HEIGHT) - ImageHeight); ///glutGet(GLUT_WINDOW_HEIGHT) - y; // ????
   mx = x / 400.0 - 1; 
   my = y / -300.0 + 1;
   ...
   printf(" coordinates of the mouse relative to upper left corner of window x=%3d  y=%3d \n", x, iY);
}
