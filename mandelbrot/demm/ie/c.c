...
/* iXmax/iYmax = 1 */
unsigned int iSide = 5000; /* side of rectangle in pixels */
...
unsigned int GivePeriodJung( double cx, double cy,  double ER2,  double Zp[2]) 
{  //determine the period, then set Zp to periodic point.
   ...
   double x=0.0L;
   double y=0.0L; // z 
   double x0, y0; // z0 inside periodic orbit
   double t; // temp
   ...
   {  
      ...
      // modified : if ( (x - x0)*(x - x0) + (y - y0)*(y - y0) < 1e-16)
...
{ 
  ...
  //double IntDistance;
     ...
     if (ExtDistance > 0.0) // exterior
       ...
       {   // period = GivePeriodJung( Cx, Cy, ER2, Zp); 
            period = GivePeriod(Cx, Cy, iMax, PixelWidth, Zp);
            //IntDistance = getInteriorLowerBound(period, Zp);
            //printf("period = %d ; IntDistance =  %f\n",period,  IntDistance);
            color = (255 - 50*period) % 255; 
            //color = iInterior;
       } 
...
int main(){
  ...
  for(iY=0;iY<iYmax;++iY){ 
    ...
    if ( fabs(Cy)<PixelHeight/2.0) Cy = 0.0;
    printf("row %u from %u \n",iY, iYmax);    
  ...
  snprintf(filename, length, "b%.0fe%u%s", EscapeRadius,iXmax  , ".pgm");
  ...
  snprintf(tfilename, length, "b%.0fe%u%s",EscapeRadius, iXmax, ".txt");
