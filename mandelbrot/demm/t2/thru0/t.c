...
void PrintInfo(double Cx, double Cy, double Cxb, double Cyb)
{
 ...
 //
   ...
   if ( i == iMax ) 
        ...
        else  printf(" ( i < iMax ) = iteration was escapes or derivative explodes  \n ");             
   if (absZ2 > ER2 ) 
        printf(" (absZ2 > ER2 ) = exterior point which simply escapes ( bailout ) \n ");
   ...
   if (EstimDistance > 0.0) 
        {
          ...
          if (absdZ2 < absZ2) 
            printf(" (absdZ2 < absZ2) = point of exterior when escaping through z=0\n "); 
            else printf(" (absdZ2 >= absZ2)  = normal for most escaping points \n ");  
         }
        else   printf(" (EstimDistance <= 0.0) = interior point \n ");        
   
   if (absdZ2 > absdZMax2) 
       printf(" (absdZ2 > absdZMax2) =  derivative explodes \n"); // 
