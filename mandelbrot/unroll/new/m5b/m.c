...
File h6.650000m6650.pgm saved. 
 save  graphic data to the text file 
File h6.650000m6650.pgm.txt saved. 
 allways free memory 
...
convert h6.650000m6650.pgm -resize 1600x240 cbig.png
...
{ 
  ...
  for (i=0; i<iMax; i++)
    { // check if not escaping : abs(z)>ER
      ...
      if (absdZ2 > 1e60) break; //  interior when derivative explodes 
      // z = fc(z) = z*z + c 
      Zy=2*Zx*Zy + Cy;
      Zx=Zx2-Zy2 +Cx;
   ...
   if (distance < 0.0000001 && distance >0.0 ) color = iBoundary ; // bondary = black
   if (distance < CDistanceMax && distance >0.0) color = (iBoundary + (int)(255*distance/CDistanceMax)) % 255; // near boundary = shades of gray
   // if (nz < bailout) return 1; //still not escaping after iteration , rare
   if (absdZ2 < absZ2) color= iExterior;  //includes escaping through 0
...
int main(){
  ...
  sprintf(name,"h%fm%u",m,iXmax); /* result (is saved in i) but is not used */
