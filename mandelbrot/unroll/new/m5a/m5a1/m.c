...
{ 
  ...
  for (i=0; i<iMax; i++)
   ...
   if (distance > 0.0) 
          { 
            if (distance < 0.000001 ) color = iBoundary ; // bondary = black
            // near boundary = shades of gray
            else if (distance < CDistanceMax ) color = (iBoundary + (int)(255*distance/CDistanceMax)) % 255; 
          }
...
int main(){
  ...
  sprintf(name,"b%fm%u",m,iXmax); /* result (is saved in i) but is not used */
