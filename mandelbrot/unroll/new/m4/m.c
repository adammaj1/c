  ...
  for (i=0; i<iMax; i++)
    { // check if not escaping : abs(z)>ER
     ...
     /* first derivative   zp = 2*z*zp  = xp + yp*i; */
      ...
      // abs 
      absZ2= Zx*Zx + Zy*Zy; // nz = x*x + y*y;
      absdZ2= dZx*dZx + dZy*dZy; // 
    };

  // if (nz < bailout) return 1; //still not escaping after iteration , rare
   if (absdZ2 < absZ) color= iExterior;  //includes escaping through 0
     else {
            temp = log(absZ);
            distance = temp*temp*absZ /(CDistanceMax*absdZ2); //4*square of dist/pixelwidth
            if (distance < 0.04) color = iBoundary ; // bondary 
            if (distance < 0.24) color = iBoudary +20; // near 
          }
