...
unsigned char iInteriorU = 200;
unsigned char iInteriorD = 255;
unsigned char iRest = 100;
...
unsigned char GiveColor(double Zx0, double Zy0,double Cx, double Cy, int iMax, double ER2, double AR2, double ZAx, double ZAy)
 {
  ...
  double d2, dX, dY; /* d = distance from z to Alpha, d2=d*d  */
 ...
 if ( d2<AR2) 
 {
   if (dX>0.0) return iInteriorU; 
              else return iInteriorD;
 }
 //else return iRest;
for (i=0; i<iMax; i++)
    { 
...
int main(){
 ...
 //stepCx = (CxMax - CxMin)/ nMax;
  ...
  const int IterationMaxBig= 1000;
