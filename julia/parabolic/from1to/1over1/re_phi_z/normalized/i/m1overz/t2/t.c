...
int main(){
  ...
  double ZxMin=0.5+dSide;
  double ZxMax=0.5+2*dSide;
  double ZyMin=-dSide/2.0;
  double ZyMax=dSide/2.0;
...
//double complex ZA;  /* atractor ZA = ZAx + ZAy*i */
 ...
 for(iY=0;iY<iYmax;++iY){ 
    ...
    for(iX=0;iX<iXmax;++iX){ 
      ...
      denominator += Zx*Zx; // 
      Zy=Zy/denominator; // inverse plane 
      //if (fabs(Zy)<PixelHeight/2) Zy=0.0; 
      Zx= -Zx/denominator; // inverse plane
