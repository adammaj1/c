...
// here z_cr = 0.0
double GiveNormalizedAR(double Cx, double Cy, double alfax, int iMax)
{
...
// printf(" Zxn = %f \n", Zx); // info 
// normalize 
 return 100000*( alfax-Zx); // multiplier to speed up computations ???? //
...
int main(){
 ...
 alfax = (1-sqrt(1-4*Cx))/2 ; /* parabolic fixed point z = alfa */
   AR = GiveNormalizedAR(Cx, Cy, alfax, IterationMaxBig);
