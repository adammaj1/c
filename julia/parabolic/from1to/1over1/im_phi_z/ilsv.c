/*
 ...
 convert -resize 1000x1000 10000000.250000000.pgm a.png
*/
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
unsigned char iInteriorImPlus = 200;
unsigned char iInteriorImMinus = 255;
...
int i;
  ...
  for (i=0; i<iMax; i++)
    { 
      ...
      // color depends on which part of target set z goes = binary decomposition of interior 
      // im (phi (z)) > 0 or < 0 
      if (d<AR2) { if (Zy>0) return iInteriorImPlus; 
                             else return iInteriorImMinus;} 
      //  forward iteration fc(z) = z^2 + c
      Zy=2*Zx*Zy + Cy;
      Zx=Zx2-Zy2 +Cx;
      //
      Zx2=Zx*Zx;
...
int main(){
 ...
 printf(" find boundaries in data array using  Sobel filter\n");   
  ...
  SavePGMFile( IterationMaxBig+Cx, data);
