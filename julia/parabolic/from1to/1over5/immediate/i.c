...
Not works , change fatou color procedure
...
unsigned int iXmax ; //
static unsigned int iWidth = 1000; // horizontal dimension of array
...
int SaveArray2PGMFile( unsigned char A[], double k, char* text)
{
 ...
 if (t==0) fprintf(fp," No lost pixels : t = %d ; Parameters iMax and AR are good \n",t);
    else fprintf(fp," Error : there are lost pixels : t = %d > 0 ; you should increase iMax or make bigger AR in setup ; check fall in test (target set) in GiveFatouColor\n\n", t); 
...
iLength = iWidth*iHeight;/* length of array in bytes = number of bytes = number of pixels of image * number of bytes of color */
 ...
 AR = PixelWidth*10.0; /*   radius of circle around attractor ZA = target set for attracting points */
 AR2  = AR*AR;
