...
unsigned int Give_iyNew (double dx, double out[3])
{
  unsigned int iy;
  double dy;
  // y = a*x^2 + bx + c 
  dy = dx*dx*out[0] + dx*out[1] + out[0];
  iy = (ZyMax - dy)/PixelHeight; // inverse Y axis 
  return iy;
}


// draws quadratic curve thru 3 points 
int dDrawQuadraticCurve(unsigned char A[],double xin[3], double yin[3], int RayNumber, int color )
{
  double out[3]; // coefficients ( out array ) or the quadratic functions thru 3 points ( in xin and yin arraya)
  unsigned int ix, iy; // screen coordinate = indices of virtual 2D array
  unsigned int ixMin;
  unsigned int iyNew;
  //double complex zNext;
  double dx;
  //unsigned int ixSteps, iySteps;

  // find coefficients ( out array ) or the quadratic functions thru 3 points ( in xin and yin arraya)
  FindFunction(xin, yin, out );
  
  //unsigned int i; // index of 1D array
  dx = xin[0];
  ixMin = (dx - ZxMin)/PixelWidth;
   
  iy = (ZyMax - yin[0])/PixelHeight; // inverse Y axis 
  printf("ix = %d ; iy = %d \n", ixMin, iy); 
  //iDrawPoint(A, ixMin, iy, color);
  //iSaveTurn( ix,iy,RayNumber);
  
 printf(" dDrawQuadraticCurve %d\n", RayNumber);
 for (ix=ixMin; ix<iAlfaX; ++ix)
  {
    if (xin[0]< xin[1]) dx += PixelWidth; else dx -= PixelWidth;
    iyNew  = Give_iyNew (dx, out);
    if ( iyNew <iyMax && iyNew>0) iDrawLine( A, ix, iy, ix+1, iyNew, RayNumber, color); 
    printf("ix = %d ; iy = %d \n", ix, iy); 
   // 
    iy = iyNew;
			    ...
			    )
{
  ...
  // arrays for 3 points used for interpolation of end part of the ray ; see dDrawQuadraticCurve 
  double xin[3];
  double xin0[iPeriodChild], yin0[iPeriodChild]; // temp 
  double yin[3];
...
//printf("t = %.10f \n", t);
 ...
 // zNext = RayXs[0] + RayYs[0] *I;
  ...
  for (iter = -10; iter <= iterMax; iter++)
	...
	{ // u+v*i = sqrt(z-c)   backward iteration in fc plane 
	 ...
	 // if (xNew>dAlfaX && ((yNew>dAlfaY && dAlfaY>RayYs[j]) || (yNew<dAlfaY && dAlfaY<RayYs[j])  ))
	  ...
	  //  }
	  //  first point for interpolarion ; see dDrawQuadraticCurve 
          if (iter==iterMax-10) 
              {  
                xin0[j]=RayXs[j];
                yin0[j]=RayYs[j];
  ...
  // approximate end part of ray by quadratic function thru 3 oints : 
  // 1.  alfa fixed point = landing point of the ray
 //  2. last point of the ray RayXs[j],RayYs[j],
 // 3. some previous point of the ray
  for (j = 0; j < n + 1; j++)
  {  // copy from tem array
     xin[0]=xin0[j];
     yin[0]=yin0[j];
     ...
     xin[1]=RayXs[j];
     yin[1]=RayYs[j];
     // used precomputed value ( in setup) 
     xin[2]=dAlfaX;
     yin[2]=dAlfaY;
     dDrawQuadraticCurve(A, xin, yin, j, 255 );
