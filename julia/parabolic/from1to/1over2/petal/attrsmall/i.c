...
int iMaxN; // normalized 
int iMaxCr; // = attracting time of critical point
  ...
  while (i<iMaxN ) 
    {
      // Fall-in test : iterate z and check when point z is inside target set.
      // Note that test is done after every f^{period} not after every f !!!
      if (d2 < ARn2)  return 100*(i % 255); // break;
...
int MarkSmallAttrPetal(unsigned char BasinColor, unsigned char A[] )
{
  //unsigned char color;
 unsigned int iX,iY, /* indices of 2D virtual array (image) = integer coordinate of the pixel */
  ...
  for(iY=0; iY<iYmax; ++iY){ 
      ...
      if (A[i]==BasinColor) // if z is in immmediate basin
        {  Zy=ZyMin + iY*PixelHeight; /* map from screen to world coordinate  */
           if (fabs(Zy)<PixelHeight/2) Zy=0.0; /*  */
           Zx=ZxMin + iX*PixelWidth;
           if (GiveAttractiveTime(Zx+Zy*I)<=iMaxCr)
               A[i]= iPetalL; /*  */
           //printf("yes");
         }
...
/*int MarkImmediateBasin( int period, unsigned char color,  unsigned char A[], unsigned char B[])*/
/*{*/
/*  double complex  Z;*/
/*  int p; */
/*  */
...
int main(){
 ...
 ComputeFatou(data);
 SaveArray2PGMFile(data,0.0+AR," Fatou components ");
 ...
 SaveArray2PGMFile(data,1.0+AR," Fatou components and Julia set ");
 ...
 MarkSmallAttrPetal(iComponentL , data );
 SaveArray2PGMFile(data,3.0+AR, "Fatou components, Julia set, immediate basin of parabolic fixed point alfa, attracting petals ");
