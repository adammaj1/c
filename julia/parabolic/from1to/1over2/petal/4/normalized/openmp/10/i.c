...
POints inside Julia set , inside immediate basin of attraction

z = -0.486666666666667  +0.146666666666667 i
...
// initiall point of iteration is a critical point of f
 ...
 for ( i=0; i<iMaxN; ++i)
  ...
  d1 = fabs(Zx- ZAx);
  if (dcr<AR && AR<d1 ) 
   { printf("  not normalised : dcr = %.20f < AR = %.20f < d1 = %.20f \n", dcr, AR, d1);
     ARn = AR;
  }
   else 
{   
    ...
    ARn = dcr + (d1-dcr)*0.1;
    // now dcr<AR<d1
    printf(" normalised parameters : dcr = %.20f < ARn = %.20f < d1 = %.20f < AR = %.20f \n", dcr, ARn, d1, AR);
 } 
 return ARn; // 
...
int SaveArray2PGMFile( unsigned char A[], double k, char* text)
{
  ...
  char comment[70]; // lines should be limited to 70 characters:  
  strncpy(comment,"#",2);/* comment should start with # */
  strncat( comment , text, 68); 
...
int FillContour(complex double seed,  unsigned char color, unsigned char A[], unsigned char B[])
{ 
  /* 
...
// uses global var : iPeriodChild, c, Zcr
int MarkImmediateBasin(  unsigned char color,  unsigned char A[], unsigned char B[])
{
  double complex  Z;
  int p; 
  int pMax = iPeriodChild; // uses globar var

  // first center is the critical point
  Z = Zcr;
  FillContour(Z , color, A,B); // fi
    
  for (p=0; p<pMax; ++p)
  {
    Z = f(Z,c); // next center is the image of critical point under f function 
    FillContour(Z , color, A,B); // fill next 
  }
...
// ??? works !!!
int IsInPetal(int iX, int iY)
{
  double Zx, Zy;
  double  Ux;
  // map from screen to world coordinate
  Zy=ZyMin + iY*PixelHeight; /* map from screen to world coordinate  */
  if (fabs(Zy)<PixelHeight/2) Zy=0.0; /*  */
  Zx=ZxMin + iX*PixelWidth;
  //
  Ux = creal( GiveNormalizedU(Zx+Zy*I, Ux0)) ; // normalised
  //
  if(Ux>=0.0) return 1;
  return 0; 
...
int MarkPetal(unsigned char color, unsigned char A[] )
{
 ...
 unsigned int iX,iY, /* indices of 2D virtual array (image) = integer coordinate of the pixel */
  ...
  #pragma omp parallel for schedule(dynamic) private(i, iX, iY) shared(iXmax, iYmax)
  for(iY=0; iY<iYmax; ++iY){ 
   
    printf(" mark petal : row %u from %u \n",iY, iYmax);    // info 
    
    for(iX=0;iX<iXmax;++iX){ 
        i= g(iX,iY); /* compute index of 1D array from indices of 2D array , range(i) = [0,iLength-1] */
      // if (i>iLength-1) printf("i = %d error on iX = %d ; iY = %d \n",i, iX , iY); // range checking 
      if (A[i]==iBasin) 
        {if (IsInPetal(iX, iY)) A[i]= color;} /*  */
...
iLength = iWidth*iHeight;/* length of array in bytes = number of bytes = number of pixels of image * number of bytes of color */
 ...
 iYmax = iHeight; // range(iY) = [0, iYmax -1]
  ...
  // for normalisation of U ( Fatou  coordinate )
  Ux0 = creal(GiveU(Zcr));
...
int main(){
 ...
 // comment line for SaveArray2PGMFile should be limited to 70 characters:  
 ...
 MarkImmediateBasin(iBasin, Fatou, Fatou );
 SaveArray2PGMFile(Fatou, 4.0+AR, "Fatou components, Julia set, immediate basin of parabolic fixed point alfa ");

 printf(" petals\n");
 MarkPetal(iPetalL , Fatou );
 SaveArray2PGMFile(Fatou, 5.0+AR, "Fatou components, Julia set, immediate basin , attracting petal ");
