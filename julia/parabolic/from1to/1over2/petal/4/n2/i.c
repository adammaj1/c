  ...
  while (i<iMax ) 
    {
      // Fall-in test : iterate z and check when point z is inside target set.
      // Note that test is done after every f^{period} not after every f !!!
      if (d2 < ARn2)  return 3*(i % 255); // break;
...
/* SaveArray2PGMFile(data,2.0+AR, "Fatou components, Julia set, immediate basin of parabolic fixed point alfa, attraction time to parabolic fixed point alfa in one attracting petal ");*/
/* */
 printf("free memory \n");
 free(data);
 free(edge);
