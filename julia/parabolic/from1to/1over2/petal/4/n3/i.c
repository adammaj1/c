  ...
  while (i<iMax ) 
    {
      // Fall-in test : iterate z and check when point z is inside target set.
      // Note that test is done after every f^{period} not after every f !!!
      if (d2 < ARn2)  {temp = i; if (temp>255) color = 255; else color= temp; return color; } ; // break;
...
int MakePetalLevels(unsigned char PetalColor, unsigned char A[] )
{
  unsigned char color;
...
/* ComputeFatou(data);*/
/* SaveArray2PGMFile(data,AR," Fatou components without boundaries");*/
...
/* SaveArray2PGMFile(data,1.0+AR," Fatou components and Julia set = common bouindary");*/
...
/* SaveArray2PGMFile(data,2.0+AR, "Fatou components, Julia set, immediate basin of parabolic fixed point alfa ");*/
/* printf(" attraction time to parabolic fixed point alfa in an attracting petal\n");*/
/* MakePetalLevels(iComponentL , data );*/
/* SaveArray2PGMFile(data,3.0+AR, "Fatou components, Julia set, immediate basin of parabolic fixed point alfa, attraction time to parabolic fixed point alfa in one attracting petal ");*/
/* */
 printf("free memory \n");
 free(data);
 free(edge);
 ...
 printf("test 0 = %d \n",test(0.0, 0.0));
 printf("test 1 = %d \n",test(0.0,PixelWidth));
 printf("test 2 = %d \n",test(0.0,2*PixelWidth));
 printf("test 20 = %d \n",test(0.0,20*PixelWidth));
 printf("F color of zcr = %d \n",GiveFatouColor(0.0, 0.0));
 printf("F color of z1 = %d \n",GiveFatouColor(0.0, PixelHeight));
 printf("color of zcr = %d \n",GivePetalLevelsColor(iWidth/2, iHeight/2));
 printf("color of z1 = %d \n", GivePetalLevelsColor(iWidth/2, 1+iHeight/2));
 return 0;
}
